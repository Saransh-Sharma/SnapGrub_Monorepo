...
- Drift stores profile, goals, body measurements, devices, feature flags, meals, meal items, templates, custom foods, daily rollups, correction events, meal assets, analysis/draft metadata, weekly insights/defaults, sync state, and outbox commands.
...
- `barcode`, `text_entry`, `voice_entry`, and label/OCR flows: create parser drafts and route them through Meal Editor.
...
- `insights`: weekly insight/default caches and feature-flagged Progress widgets.
...
- Keep Phase 5 barcode/OCR/text/voice parser output as editable drafts until the user confirms in Meal Editor.
- Photo analysis output must enter the existing Meal Editor as an editable draft, not a parallel save path.
- Pause/resume camera preview through app lifecycle and refresh permission status when Home appears.
- Surface failed/conflict sync state from Home and link to the Sync status recovery screen.
