...
| `api_rate_limits` | no direct client access; service-role RPC only |

## Storage RLS

| Bucket | Client access |
| --- | --- |
| `meal-originals-private` | authenticated users upload/read objects under their own user-ID prefix |
| `meal-thumbnails-private` | authenticated users upload/read objects under their own user-ID prefix |
| `exports-private` | authenticated users can read own-prefix objects through storage RLS, but artifacts are generated by service-role functions and downloaded through signed URLs |

`account-delete` and `media-retention-cleanup` use service-role storage access because they must remove private objects across buckets.
