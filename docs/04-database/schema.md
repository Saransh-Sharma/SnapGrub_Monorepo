...
Migration SQL is the final source of truth. This page summarizes the current Phase 0-8 schema so developers can understand ownership, access patterns, and safe changes.
...
- `mark_expired_exports_failed`: marks expired export artifacts and returns paths for storage cleanup.
