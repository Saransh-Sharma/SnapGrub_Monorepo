...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const job = await persistAnalysis(client, user.id, clientRequestId, "text", body, result);
...
async function persistAnalysis(
  ...
  const resultPayload = {
    ...result,
    provenance: { ...result.provenance, analysis_id: job.id },
  };
