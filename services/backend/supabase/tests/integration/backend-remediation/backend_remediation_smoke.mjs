...
function fakeServiceRoleJwt() {
  const header = btoa(JSON.stringify({ alg: 'none', typ: 'JWT' })).replaceAll('+', '-').replaceAll('/', '_').replaceAll('=', '');
  const payload = btoa(JSON.stringify({ role: 'service_role' })).replaceAll('+', '-').replaceAll('/', '_').replaceAll('=', '');
...
try {
  ...
  assert(spoofResponse.status === 401, 'unsigned service_role JWT must not authorize cleanup.');
  ...
  assert(firstBootstrap.data.device.install_id === installId, 'first device bootstrap should succeed.');

  const secondBootstrap = await invoke(userB.client, 'profile-bootstrap', {
    body: { install_id: installId, platform: 'ios', timezone: 'UTC' },
  });
  assert(secondBootstrap.error || secondBootstrap.data?.code === 'CONFLICT', 'install_id takeover must be rejected.');
  ...
  const deletedRead = await invoke(userA.client, `meals/${mealId}`, { method: 'GET' });
  assert(deletedRead.error || deletedRead.data?.code === 'NOT_FOUND', 'soft-deleted meal detail should return not found.');

  const { data: userBCustomFood, error: customFoodError } = await admin.from('custom_foods').insert({
    user_id: userB.id,
    client_id: crypto.randomUUID(),
  ...
  assert(crossUserCustomFoodMeal.error || crossUserCustomFoodMeal.data?.code === 'INVALID_INPUT', 'meal write must reject another user custom_food_id.');
  ...
  assert(firstBadSettings.error || firstBadSettings.data?.code === 'INVALID_INPUT', 'invalid settings input should fail validation.');
  const secondBadSettings = await invoke(userA.client, 'settings-patch', {
    method: 'PATCH',
    headers: { 'Idempotency-Key': idempotencyKey },
    body: badSettingsBody,
  });
  assert(secondBadSettings.error || secondBadSettings.data?.code === 'INVALID_INPUT', 'invalid settings retry must not become in-progress idempotency conflict.');
