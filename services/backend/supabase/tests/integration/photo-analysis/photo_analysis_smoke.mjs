...
try {
  ...
  assert(
    ...
    "photo analysis should complete with mock provider",
  );
  assert(
    analysis.asset_id,
    "photo analysis should return a persisted asset id",
  );
  assert(
    analysis.result?.title,
    "photo analysis should return an editable draft",
  );
  assert(
    analysis.result?.confidence?.warnings?.length >= 1,
    "photo analysis should include review warnings",
  );
  ...
  assert(
    ...
    "photo analysis should log one model invocation",
  );
  assert(
    invocations[0].status === "completed",
    "model invocation should be completed",
  );

  const recovered = await invokeOrThrow(
    anon,
    `analysis-get/${analysis.analysis_id}`,
    {
      method: "GET",
    },
  );
  assert(
    recovered.analysis_id === analysis.analysis_id,
    "analysis-get should recover the analysis by id",
  );
  assert(
    recovered.result?.title === analysis.result.title,
    "analysis-get should return the latest result payload",
  );

  const badPath = `${crypto.randomUUID()}/not-owned.jpg`;
  const { data: rejected, error: rejectError } = await anon.functions.invoke(
  ...
  const rejectedCode =
    rejected?.error?.code ??
    rejected?.code ??
    (await functionErrorCode(rejectError));
  assert(
    rejectedCode === "INVALID_INPUT",
    `cross-user storage path should be rejected with INVALID_INPUT, got: ${JSON.stringify({ rejected, rejectedCode })}`,
  );

  const { data: badThumb, error: badThumbError } = await anon.functions.invoke(
    "analysis-photo-create",
    {
      body: {
        client_request_id: crypto.randomUUID(),
  ...
  const badThumbCode =
    badThumb?.error?.code ??
    badThumb?.code ??
    (await functionErrorCode(badThumbError));
  assert(
    badThumbCode === "INVALID_INPUT",
    `cross-user thumbnail path should be rejected with INVALID_INPUT, got: ${JSON.stringify({ badThumb, badThumbCode })}`,
  );
