...
async function invokeExpectError(client, functionName, options, expectedCode) {
  const { data, error } = await client.functions.invoke(functionName, options);
  let body = data ?? null;
  if (
    !body?.code &&
...
try {
  ...
  assert(
    exported.export_request.status === "completed",
    "export should complete synchronously",
  );
  assert(
    exported.export_request.result_storage_path,
    "export should have a storage path",
  );
  assert(
    exported.export_request.signed_url,
    "export should return a signed URL",
  );
  assert(
    exported.export_request.row_counts.meals === 1,
    "export should count meal rows",
  );

  const recovered = await invokeOrThrow(
    exportUser.client,
    `exports-create/${exported.export_request.id}`,
    {
      method: "GET",
    },
  );
  assert(
    recovered.export_request.id === exported.export_request.id,
    "export polling should return the same export",
  );
  ...
  assert(
    replayed.export_request.id === exported.export_request.id,
    "export idempotency should replay completed response",
  );
  ...
  assert(
    typeof cleanup.export_cleanup.expired_requests === "number",
    "cleanup should return export counts",
  );
