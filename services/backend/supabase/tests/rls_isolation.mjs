...
try {
  ...
  for (const user of [userA, userB]) {
    ...
    const { error: goalError } = await user.client.from('nutrition_goals').insert({
      user_id: user.id,
      goal_type: 'lose',
      calories_kcal: 1900,
      protein_g: 130,
  ...
  const { data: updatedOwnProfile, error: ownProfileUpdateError } = await userA.client
    .from('profiles')
  ...
  const { data: otherProfile } = await userA.client.from('profiles').select('*').eq('id', userB.id);
  assert(otherProfile.length === 0, 'User A must not read User B profile.');

  const { data: updatedOtherProfile, error: otherProfileUpdateError } = await userA.client
    .from('profiles')
    .update({ display_name: 'Leaked update' })
    .eq('id', userB.id)
    .select('*');
  if (otherProfileUpdateError) throw otherProfileUpdateError;
  assert(updatedOtherProfile.length === 0, 'User A must not update User B profile.');
  ...
  const { data: otherGoals } = await userA.client.from('nutrition_goals').select('*').eq('user_id', userB.id);
  assert(otherGoals.length === 0, 'User A must not read User B goals.');

  const { data: updatedOtherGoals, error: otherGoalUpdateError } = await userA.client
    .from('nutrition_goals')
    .update({ calories_kcal: 2100 })
    .eq('user_id', userB.id)
    .select('*');
  if (otherGoalUpdateError) throw otherGoalUpdateError;
  assert(updatedOtherGoals.length === 0, 'User A must not update User B goals.');

  const { data: updatedOwnMeasurements, error: ownMeasurementUpdateError } = await userA.client
  ...
  const { data: updatedOwnDevices, error: ownDeviceUpdateError } = await userA.client
    .from('devices')
  ...
  const { data: otherDevices } = await userA.client.from('devices').select('*').eq('user_id', userB.id);
  assert(otherDevices.length === 0, 'User A must not read User B devices.');

  const { data: flags } = await userA.client.from('feature_flags').select('*').eq('key', 'rls.test.enabled');
  assert(flags.length === 1, 'Authenticated users should read global feature flags.');

  const { data: overrides } = await userA.client.from('feature_flag_overrides').select('*');
  assert(overrides.length === 0, 'Users must not read feature flag overrides directly.');

  const { data: events } = await userA.client.from('analytics_events').select('*');
  ...
  const { data: otherItems } = await userA.client.from('meal_items').select('*').eq('user_id', userB.id);
  assert(otherItems.length === 0, 'User A must not read User B meal items.');

  const { data: otherTemplates } = await userA.client.from('meal_templates').select('*').eq('user_id', userB.id);
  assert(otherTemplates.length === 0, 'User A must not read User B meal templates.');

  const { data: otherCustomFoods } = await userA.client.from('custom_foods').select('*').eq('user_id', userB.id);
  assert(otherCustomFoods.length === 0, 'User A must not read User B custom foods.');

  const { data: otherRollups } = await userA.client.from('daily_rollups').select('*').eq('user_id', userB.id);
  assert(otherRollups.length === 0, 'User A must not read User B daily rollups.');
